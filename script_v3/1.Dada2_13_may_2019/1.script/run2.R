...
dada2 <- function(path, run){
  ...
  # csv
  write.table(t(seqtab),  "seqtab2.csv", sep=";", dec=",")
  write.table(t(seqtab.nochim),  "seqtabnochim2.csv", sep=";", dec=",")
  saveRDS(seqtab.nochim, file="seqtab2.rds")
  seqtabnochim2 <<- seqtab.nochim
  print("Seqtable done.")
  ...
  # csv
  write.table(track,"stats2.csv",sep=";",dec=",") 
  saveRDS(track, file="stats2.rds")
  stats2 <<- track
  print("Stats done.")
  ...
  #head(taxa.print)
  write.table(taxa,"taxa2.csv",sep=";",dec=",")
  saveRDS(taxa, file="taxa2.rds")
  taxa2 <<- taxa
  print ("Taxonomy done.")
...
plot_folder(path, "plot-quality2")

pdf("1-plotForward2.pdf")
plotQualityProfile(fwd, aggregate = TRUE) #forward reads
dev.off()

pdf("2-plotReverse2.pdf")
plotQualityProfile(rvs, aggregate = TRUE) #reverse reads
dev.off()
 

plot_folder(path, "plot-error2")

pdf("3-plotErrorsFwd2.pdf")
plotErrors(errFwd, nominalQ=TRUE)
dev.off()

pdf("4-plotErrorsRvs2.pdf")
plotErrors(errRvs, nominalQ=TRUE)
dev.off()

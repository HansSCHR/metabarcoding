...
plot_folder(path, "plot-quality1")

pdf("1-plotForward1.pdf")
plotQualityProfile(fwd, aggregate = TRUE) #forward reads
dev.off()

pdf("2-plotReverse1.pdf")
plotQualityProfile(rvs, aggregate = TRUE) #reverse reads
dev.off()


plot_folder(path, "plot-error1")

pdf("3-plotErrorsFwd1.pdf")
plotErrors(errFwd, nominalQ=TRUE)
dev.off()

pdf("4-plotErrorsRvs1.pdf")
plotErrors(errRvs, nominalQ=TRUE)
dev.off()
